    ...
    def __init__(self,systemsize_x, systemsize_y,cell=tf.contrib.rnn.LSTMCell,activation=tf.nn.relu,units=[10],scope='RNNwavefunction',seed = 111):
        ...
        with self.graph.as_default():
            with tf.variable_scope(self.scope,reuse=tf.AUTO_REUSE):
              self.lstm=[tf.nn.rnn_cell.MultiRNNCell([cell(units[n]) for n in range(len(units))])]*(self.Nx*self.Ny)
              ...
              self.dense = [tf.layers.Dense(2,activation=tf.nn.softmax,name='wf_dense', dtype = tf.float64)]*(self.Nx*self.Ny)
    ...
    def sample(self,numsamples,inputdim):
        ...
        with self.graph.as_default(): #Call the default graph, used if willing to create multiple graphs.
            with tf.variable_scope(self.scope,reuse=tf.AUTO_REUSE):
                ...
                lstm_state=self.lstm[0].zero_state(self.numsamples,dtype=tf.float64)
                ...
                for ny in range(self.Ny): #Loop over the number of sites
                  for nx in range(self.Nx):
                    lstm_output, lstm_state = self.lstm[ny*Nx+nx](inputs, lstm_state)
                    output=self.dense[ny*Nx+nx](lstm_output)
    ...
    def log_probability(self,samples,inputdim):
        ...
        with self.graph.as_default():
            ...
            with tf.variable_scope(self.scope,reuse=tf.AUTO_REUSE):
                ...
                lstm_state=self.lstm.zero_state[0](self.numsamples,dtype=tf.float64)
