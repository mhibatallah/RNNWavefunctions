...
wf=RNNwavefunction(Nx,Ny,units=units,cell=tf.contrib.cudnn_rnn.CudnnCompatibleGRUCell, seed = seed) #contains the graph with the RNNs
...
with tf.variable_scope(wf.scope,reuse=tf.AUTO_REUSE):
    with wf.graph.as_default():
      ...
      for it in range(len(meanEnergy),numsteps+1):
        ...
        # if it%500==0 and varE <= np.min(varEnergy[1::500]):
        #   #Saving the performances if the model is better
        #   saver.save(sess,path+'/'+filename)
        if it%500==0 and varE <= np.min(varEnergy[::500]):
          #Saving the performances if the model is better
          saver.save(sess,path+'/'+filename)
