...
def tensordot(tf,
  ...
  def _tensordot_reshape(
    ...
    if a.get_shape().ndims is not None and isinstance(axes, (list, tuple)):
      ...
      axes = tf.convert_to_tensor(value=axes, dtype=tf.dtypes.int32, name="axes")
      free = tf.convert_to_tensor(value=free, dtype=tf.dtypes.int32, name="free")
      shape_a = tf.shape(input=a)
      transposed_a = _tranpose_if_necessary(a, perm)
    else:
      ...
      rank_a = tf.rank(a)
      axes = tf.convert_to_tensor(value=axes, dtype=tf.dtypes.int32, name="axes")
      axes = tf.compat.v1.where(axes >= 0, axes, axes + rank_a)
      ...
      transposed_a = tf.transpose(a=a, perm=perm)

    free_dims = tf.gather(shape_a, free)
    axes_dims = tf.gather(shape_a, axes)
    prod_free_dims = tf.reduce_prod(input_tensor=free_dims)
    prod_axes_dims = tf.reduce_prod(input_tensor=axes_dims)
  ...
  def _tensordot_axes(a: Tensor, axes
    ...
    axes = tf.convert_to_tensor(value=axes, name="axes", dtype=tf.int32)
  ...
  with tf.compat.v1.name_scope(name, "Tensordot", [a, b, axes]) as _name:
    ...
    a_free_dims = tf.convert_to_tensor(value=a_free_dims, dtype=tf.dtypes.int32)
    b_free_dims = tf.convert_to_tensor(value=b_free_dims, dtype=tf.dtypes.int32)
    product = tf.reshape(
